# Dziennik decyzji: problem, rozwiązanie, wynik

Pełny zapis pracy nad projektem [Chatbot RAG](README.md): co nie działało, co z tym zrobiłem
i co pokazał pomiar. Każda pozycja opisuje jedną decyzję, razem z tymi, które pomiar odrzucił.

Skrócony obraz systemu jest w [README.md](README.md). Ten plik jest długi celowo, to materiał
źródłowy, nie wizytówka.

Pełne logi pomiarowe są prowadzone lokalnie w katalogu `Pomiary/` i nie trafiają do repozytorium,
bo zawierają surowe przebiegi i dane z logu produkcyjnego. Nazwy plików w tekście wskazują,
gdzie leży log konkretnego pomiaru u mnie na dysku.
...
### 23. Automatyczny routing sekcji usunięty z produkcji

**Problem.** Po sekcjach 20 i 21 na ścieżce żądania został cały aparat zgadywania strony: prior leksykalny, funkcja rozstrzygająca między stronami, wspólna pula `all` i wyłączony klasyfikator LLM. Żaden z tych elementów nie wygrał pomiaru, a odnotowany na końcu sekcji 21 problem był realny: prior potrafił zostać odwrócony przez surowy wynik rerankera dla drugiej strony.

**Rozwiązanie.** Stronę wybiera wyłącznie użytkownik, przełącznikiem w interfejsie, domyślnie kupujący. Gdy pierwsza sekcja odmówi, ten sam łańcuch bramek leci jeszcze raz na drugiej sekcji, a udana odpowiedź dostaje notę o zamianie sekcji. Wspólna pula `all` zniknęła ze ścieżki żądania, funkcja prioru przeniesiona do `Pomiary/` jako materiał do klasyfikatora forum.

**Wynik.** Zero zgadywania strony i mniej ruchomych części na ścieżce żądania. Cena: użytkownik, który nie wie po której jest stronie, płaci jeden dodatkowy przebieg retrievalu przy odmowie z pierwszej sekcji. Ta cena jest widoczna w ogonie latencji, nie w medianie.

### 24. Panel analityczny, oceny i licznik kosztów

**Problem.** Log JSONL istniał od sekcji 12, ale jedynym wglądem była strona w Streamlicie, a przede wszystkim nie było jak powiedzieć, która konkretna odpowiedź była zła. Bez tego nie da się zamknąć pętli poprawy jakości.

**Rozwiązanie.** Kciuk góra i dół przy odpowiedzi (`POST /ocena`), z identyfikatorem zapytania łączącym ocenę z wpisem odpowiedzi w logu. Panel pod `/admin`: statystyki z filtrami po dniach, języku i stronie, lista przypadków ocenionych z automatyczną diagnozą powodu, eksport do CSV i JSON z zabezpieczeniem przed formułami w arkuszu, reset archiwizujący log zamiast go kasować. Do tego middleware liczące tokeny i koszt każdego żądania oraz limity per IP obok limitu globalnego.

**Wynik.** Narzut middleware kosztów zmierzony metodą, która sama w sobie była wnioskiem. Dwie osobne serie jedna po drugiej dały 1,20 ms i 7 procent, co wyglądało na realny koszt. Ta sama zmiana mierzona przeplatanymi parami, z naprzemienną kolejnością w parze, dała minus 0,125 ms przy podłodze szumu A/A 0,236 ms, czyli poniżej rozdzielczości pomiaru. Cała różnica pierwszej wersji to był dryf maszyny wpisany prosto w wynik.

### 25. Reranker: tytuł w parze, okno 192, dwunastu kandydatów

**Problem.** Rozbicie czasu retrievalu pokazało, że reranker to 92,9 procent kosztu: 2064 ms z 2221 ms mediany, przy 133 ms na embedding i 33 ms na oba rankingi z RRF.

**Rozwiązanie.** Trzy zmiany jednym commitem, bo działają tylko razem: tytuł artykułu doklejony do fragmentu w parze ocenianej przez cross-encoder, okno skrócone z 512 do 192 tokenów, liczba kandydatów z 20 do 12. Próg rerankera obniżony z −4.3 do −5.7, bo tytuł w parze przesuwa całą skalę wyników w dół.

**Wynik.** Konfiguracja końcowa bije poprzednią na każdej osi jakości naraz i jest 3,81 raza szybsza.

| Wariant | Mediana | Przyspieszenie | top 1 | top 3 | top 5 | Odstęp median |
|---|---|---|---|---|---|---|
| 512, sam tekst, k20 | 2762 ms | 1,00 | 0,543 | 0,671 | 0,729 | 5,63 |
| **192 + tytuł, k12** | **725 ms** | **3,81** | **0,600** | **0,700** | **0,743** | **7,58** |

To 2037 ms zdjęte z ówczesnej mediany 5860 ms, czyli 35 procent czasu odpowiedzi. Próg −5,7, a nie −5,17: ta druga wartość to najniższy wynik w zestawie golden, czyli próg wpasowany dokładnie w jedno pytanie. Zapas 0,49, taki sam jak trzymała dotychczasowa produkcja, daje −5,66, w zaokrągleniu −5,7.

### 26. Strumień odmowy: sędzia równolegle z generacją, bufor optymistyczny (w toku)

**Problem.** Sędzia kontekstu jest bramką przed odpowiedzią, więc dopóki nie wydał werdyktu, użytkownik nie widzi ani jednego tokenu. Osobno: bramki po generacji straciły w międzyczasie możliwość wycofania odpowiedzi, która już poszła do przeglądarki.

**Rozwiązanie.** Sędzia startuje w puli wątków równolegle z generacją, a nie przed nią. Pierwsze 40 tokenów czeka w buforze na werdykt: przy TAK bufor leci do użytkownika i strumień dalej płynie, przy NIE strumień jest zamykany, a system odmawia. Po przekroczeniu bufora odpowiedź idzie optymistycznie dalej, a negatywny werdykt trafia już tylko do cech w logu, bez kasowania gotowej odpowiedzi. Bramki po generacji odzyskały wycofanie przez zdarzenie `reset`, które każe klientowi wyczyścić to, co już pokazał. Porzucone zapytanie anuluje zadanie sędziego, o ile ono jeszcze nie ruszyło.

**Wynik.** Praca w toku, na osobnej gałęzi, 305 testów zielonych. Znane ograniczenie, zmierzone wcześniej: gdy zadanie sędziego już ruszyło, anulowanie nie zwalnia slotu serwera modelu (1815 ms wobec podłogi 35 ms), bo `Future.cancel()` nie przerywa pracy, która się zaczęła.
...
**Po usunięciu podziału na sekcje i rozszerzeniu zestawów** (30→61 trafnych, 18→29 spoza tematu): próg rerankera −3.2 → −4.3, pokrycie 0.40 → 0.20.

**Po przyspieszeniu rerankera** (sekcja 23): próg rerankera −4.3 → −5.7, pokrycie bez zmian. Tytuł doklejony do fragmentu przesuwa całą skalę wyników rerankera w dół, więc stary próg odrzucałby pytania, na które system dziś odpowiada. Stan obecny: PL −5.7 / 0.20, EN −3.6 / 0.35.
...
| embedding | 0.07 s |
| wyszukiwanie | 0.19 s |
| reranking, konfiguracja sprzed sekcji 23 | 1.6 s |
| reranking, konfiguracja obecna (192 + tytuł, k12) | 0.36 s |
