...
def test_pusty_tekst_zero():
    assert pipeline.pokrycie_idf('', [_chunk('konto haslo')]) == 0.0


def test_pelne_pokrycie_wysokie():
    wynik = pipeline.pokrycie_idf('konto haslo', [_chunk('konto haslo zwrot')])
    assert wynik == pytest.approx(1.0)


def test_brak_wspolnych_lematow_niski():
    wynik = pipeline.pokrycie_idf('zwrot', [_chunk('konto haslo')])
    assert wynik == 0.0


def test_nieznany_lemat_dostaje_idf_max():
    wynik = pipeline.pokrycie_idf('niecodwiadomy', [_chunk('niecodwiadomy inny tekst')])
    assert wynik == pytest.approx(1.0)
